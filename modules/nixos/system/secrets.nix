...

{
  ...
  sops.secrets.openvpn-work = {
    sopsFile = "${secretspath}/secrets/ceris-vpn.yaml";
    key = "openvpn";
  };

  sops.secrets."opalbolt/ssh-key-pub" = {
    owner = config.users.users.mads.name;
    path = "/home/mads/.ssh/id_ed25519.pub";
  };
  sops.secrets."opalbolt/ssh-key-priv" = {
    owner = config.users.users.mads.name;
  ...
  sops.secrets.ca_cert = {
    owner = config.users.users.mads.name;
    sopsFile = "${secretspath}/secrets/ceris-wifi.yaml";
  };
