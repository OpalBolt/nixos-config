{ inputs, ... }:
{
  # Work-specific HM config: apps, secrets management via rclone, audit tooling.
  # Parametric: uses host.name to derive per-host sops secret file paths.
  den.aspects.work.includes = [
    ({ host, ... }: {
      homeManager =
          ...
          workAuditScript = pkgs.writeShellScriptBin "work-audit" ''
            ...
            if [ "$UPLOAD" = true ]; then
                ...
                if ${pkgs.curl}/bin/curl -g -T "$FILEPATH" -u "$USERNAME:$PASSWORD" -H 'X-Requested-With: XMLHttpRequest' https://eficloud.eficode.com/public.php/webdav/; then
                    echo "$(date): Upload of $FILENAME - SUCCESS" >> "${reportDir}/upload_status.log"
                    echo "Upload successful."
                else
                    echo "$(date): Upload of $FILENAME - FAILURE" >> "${reportDir}/upload_status.log"
                    echo "Upload failed."
                    exit 1
                fi
            else
        ...
        in
        {
          ...
          home.packages = lib.flatten [
            ...
            (with pkgs; [ slack timewarrior awscli2 lynis curl workAuditScript ])
          ];
          home.shellAliases.th = "time-helper";
          programs.zsh.initContent = ''
            eval "$(renv shell-init)"
            eval "$(kctx shell-init)"
          '';

          # Lynis audit sops secrets
          sops.secrets = {
          ...
          systemd.user.timers.work-audit = {
            Unit.Description = "Trigger work-audit on the last day of every month at 08:00";
            Timer = {
          ...
          # Google Drive mount for customer files via rclone
          sops.secrets."rclone/client_id".sopsFile = "${secretspath}/secrets/${hostname}-rclone.yaml";
          sops.secrets."rclone/client_secret".sopsFile = "${secretspath}/secrets/${hostname}-rclone.yaml";
          sops.secrets."rclone/token".sopsFile = "${secretspath}/secrets/${hostname}-rclone.yaml";
          sops.secrets."rclone/team_drive".sopsFile = "${secretspath}/secrets/${hostname}-rclone.yaml";
          ...
          programs.rclone = {
            ...
            remotes.gdrive = {
              ...
              secrets = {
                ...
                team_drive = config.sops.secrets."rclone/team_drive".path;
              };
              mounts.Customers = {
                enable = true;
